...
void ecrire_image(Image I)
{
    ...
    for (y = 1; y <= hauteur_image(I); y++)
    {
        ...
        {
            printf("%c ", get_pixel_image(I, x, y) == NOIR ? '1' : '0');
        }
        printf("\n");
    }
}
...
{
    UINT x, y;
    Image negatif_I = creer_image(largeur_image(I), hauteur_image(I));

    for (y = 1; y <= hauteur_image(I); y++)
    {
        for (x = 1; x <= largeur_image(I); x++)
        {
            Pixel pixel = get_pixel_image(I, x, y);
            set_pixel_image(negatif_I, x, y, pixel == NOIR ? BLANC : NOIR);
        }
    }
